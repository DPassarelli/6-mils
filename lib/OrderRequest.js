...
class OrderRequest extends OutboundMessage {
  ...
  constructor (options) {
    ...
    props.order.date = (
      options.orderDate
        ? moment(options.orderDate, moment.ISO_8601, true).local().format()
        : moment().local().format()
    )
  ...
  get orderId () {
    return _private.get(this).order.id
  }

  set orderId (newValue) {
    const props = _private.get(this)
    props.order.id = (newValue == null ? '' : newValue.toString())

    _private.set(this, props)
  }

  get orderDate () {
    return _private.get(this).order.date
  }
