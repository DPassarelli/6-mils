...
class OrderRequest extends OutboundMessage {
  ...
  setShippingInfo (options) {
    ...
    // props.shipTo.address = merge({ attn: [] }, options.address) // this avoids any chance of modifying the input value
    ...
    if (Array.isArray(options.address.attentionOf)) {
      ...
      options.address.attn = options.address.attentionOf
    } else {
      options.address.attn = [options.address.attentionOf]
    }

    options.address.nickname = options.address.nickname || 'default'
