  ...
  describe('instance members', function () {
    ...
    describe('setSupplierInfo', function () {
      ...
      it('must populate the "Sender" element in the cXML message', function () {
        const expected = /<To><Credential domain="%%SENDER_DOMAIN%%"><Identity>%%SENDER_IDENTITY%%<\/Identity><\/Credential><\/To>/
        const actual = instance.setSupplierInfo({ domain: '%%SENDER_DOMAIN%%', id: '%%SENDER_IDENTITY%%' }).toString()

        expect(actual).to.match(expected)
      })

      it('must populate the "Sender" element correctly if called more than once', function () {
        const expected = /<To><Credential domain="%%SENDER_DOMAIN_2%%"><Identity>%%SENDER_IDENTITY_2%%<\/Identity><\/Credential><\/To>/
