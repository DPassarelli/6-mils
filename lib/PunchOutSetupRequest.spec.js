...
describe('the "PunchOutSetupRequest" module', function () {
  ...
  describe('the constructor parameters', function () {
    it('must insert the payload identifier, if fully specified', function () {
      ...
      return getSampleXml({ payloadId: 'test-test.1.2.3@whoknowswhere' })
        .then(function (expected) {
          const actual = instance.toString()
          expect(actual).to.equal(expected)
        })
    ...
    it('must insert the payload identifier with control character escaping', function () {
      const instance = new T({ payloadId: '"></cXML>some malicious content' })

      return getSampleXml({ payloadId: '&quot;&gt;&lt;/cXML&gt;some malicious content' })
        .then(function (expected) {
          const actual = instance.toString()
          expect(actual).to.equal(expected)
        })
