      ...
      return getSampleXml()
        .then(function (expected) {
          const actual = instance
            ...
            .replace(/<UserAgent>.+<\/UserAgent>/, '<UserAgent></UserAgent>')
            .replace(/<BuyerCookie>\S+<\/BuyerCookie>/, '<BuyerCookie></BuyerCookie>')

          expect(actual).to.equal(expected)
        })
    })

    it('must return an formatted empty template if no other methods are called, and the "format" option is specified', function () {
  ...
  describe('instance members', function () {
    ...
    describe('setBuyerInfo', function () {
      ...
      it('must retain parameter values from previous invocations', function () {
        const expected = /<From><Credential domain="%%FROM_DOMAIN%%"><Identity>%%FROM_IDENTITY_2%%<\/Identity><\/Credential><\/From>/
        const actual = instance
    ...
    describe('setSupplierInfo', function () {
      ...
      it('must retain parameter values from previous invocations', function () {
        const expected = /<To><Credential domain="%%TO_DOMAIN%%"><Identity>%%TO_IDENTITY_2%%<\/Identity><\/Credential><\/To>/
        const actual = instance
    ...
    describe('setSenderInfo', function () {
      ...
      it('must populate the "Sender" element in the cXML message', function () {
        const expected = /<Sender><Credential domain="%%SENDER_DOMAIN%%"><Identity>%%SENDER_IDENTITY%%<\/Identity><SharedSecret>%%SECRET%%<\/SharedSecret><\/Credential><UserAgent>.+<\/UserAgent><\/Sender>/
      ...
      it('must populate the "Sender" element correctly if called more than once', function () {
        const expected = /<Sender><Credential domain="%%SENDER_DOMAIN_2%%"><Identity>%%SENDER_IDENTITY_2%%<\/Identity><SharedSecret>%%SECRET_2%%<\/SharedSecret><\/Credential><UserAgent>.+<\/UserAgent><\/Sender>/
      ...
      it('must convert missing values to empty strings', function () {
        const expected = /<Sender><Credential domain=""><Identity><\/Identity><SharedSecret><\/SharedSecret><\/Credential><UserAgent>.+<\/UserAgent><\/Sender>/
      ...
      it('must convert non-string values to strings', function () {
        const expected = /<Sender><Credential domain="3.14159"><Identity>\[object Object\]<\/Identity><SharedSecret>Symbol\(TEST\)<\/SharedSecret><\/Credential><UserAgent>.+<\/UserAgent><\/Sender>/
      ...
      it('must retain parameter values from previous invocations', function () {
        const expected = /<Sender><Credential domain="%%SENDER_DOMAIN%%"><Identity>%%SENDER_IDENTITY_2%%<\/Identity><SharedSecret>%%SECRET%%<\/SharedSecret><\/Credential><UserAgent>.+<\/UserAgent><\/Sender>/
        const actual = instance
      ...
      it('must reset parameter values if all null values are passed', function () {
        const expected = /<Sender><Credential domain=""><Identity><\/Identity><SharedSecret><\/SharedSecret><\/Credential><UserAgent>\S+<\/UserAgent><\/Sender>/
        const actual = instance
...
/**
 * TODO: POSRes must include timestamp
 */
})
