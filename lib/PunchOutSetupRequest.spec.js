...
describe('the "PunchOutSetupRequest" module', function () {
  ...
  describe('control character encoding', function () {
    it('must be performed on the value for "payloadID"', function () {
      const instance = new T({ payloadId: '"whoknows<where>&"' })
      const expected = /<cXML.*payloadID="&quot;whoknows&lt;where&gt;&amp;&quot;"[^>]*>/
      const actual = instance.toString()
      expect(actual).to.match(expected)
    })
