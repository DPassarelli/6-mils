...
class OutboundMessage {
   ...
   *
   * @param {Object}   exts   A dictionary of key-value pairs that will be used
  ...
  setExtrinsic (exts) {
    ...
    if (!isPlainObject(exts)) {
      throw new Error('The "exts" parameter, if provided, must be a plain object.')
    }

    const props = _private.get(this)
    props.extrinsics = exts
  ...
  generateCxml (props, format) {
    ...
    const cxml = nunjucks.render(baseProps.templateName, merge(props, baseProps))
