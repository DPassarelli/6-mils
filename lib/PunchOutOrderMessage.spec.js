...
const testsForAllCxmlMessages = require(path.join(
  __dirname,
  '../test/shared-unit/cxml.js'
))
const testsForInboundMessages = require(path.join(
  __dirname,
  '../test/shared-unit/inbound.js'
))
...
const SAMPLE_CXML = fs
  .readFileSync(
    path.join(__dirname, '../test/samples/PunchOutOrderMessage.xml')
  )
  .toString()
...
describe('the "PunchOutOrderMessage" module', function () {
  ...
  describe('the instance members specific for this class', function () {
    ...
    context(
      'when the PunchOutOrderMessage contains "ItemIn" elements',
      function () {
          ...
          it('must match the values provided in the "<ItemIn>" element', function () {
            const expected = [
              ...
              },
              {
                ...
                currency: 'EUR',
                description: 'Leather Reclining Desk Chair without Padded Arms',
                name: 'Leather Reclining Desk Chair without Padded Arms',
                quantity: 1,
                supplierPartId: '4444',
                supplierPartAuxId: 'E000028902',
                unitPrice: 227.34,
                uom: 'EA'
              }
            ]
    ...
    context(
      'when the PunchOutOrderMessage does not contain any "ItemIn" elements',
      function () {
        const modifiedCxml = SAMPLE_CXML.replace(
          /<ItemIn quantity="\d+">[^]+<\/ItemIn>/,
          ''
        ).replace(
          /<PunchOutOrderMessageHeader[^>]+>[^]+<\/PunchOutOrderMessageHeader>/,
          ''
        )
